...
def format_excel():
    ...
    return 'excel exported and formatted!'
...
def import_tmrs():
    imported_tmrs = {}
    # reading excel
    df1 = pd.read_excel('tmr_submitted_bin.xls')
    ...
    if exists('UMCC Tracker.csv'):
        ...
        for ind, n in enumerate(imported_tmr_dict.values()):
            if n['tmr num'] in existing_list:
                ...
                # print(f"found {n[1]} at index {tmr_info}")
                name = n['tmr name']
                sd = n['start dtg']
        ...
        for ind, n in enumerate(imported_tmr_dict.values()):
                ...
                support_needed = input(f'Enter support submitted for {n[0]} {n[1]}, from {n[3]} -> {n[5]}: ')
                comments = input(f'Enter any additional comments for {n[0]} {n[1]}: ')
                print('\n')
...
# these are actual logic functions
imported = import_tmrs()
current_tmr = remove_old(imported)
results = import_movements(current_tmr)


# these are used to sylize the excel sheet and csv file
export_movement_tracker(results)
format_excel()